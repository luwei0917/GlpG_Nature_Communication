...
FixSpringLipidFluctuations::FixSpringLipidFluctuations(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  group2(NULL)
{
  ...
  if (strcmp(arg[3],"couple") == 0) {
    if (narg != 11) error->all(FLERR,"Illegal fix spring command");
    styleflag = COUPLE;
...
FixSpringLipidFluctuations::~FixSpringLipidFluctuations()
{
  delete [] group2;
}
...
int FixSpringLipidFluctuations::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
  mask |= THERMO_ENERGY;
...
void FixSpringLipidFluctuations::init()
{
...
void FixSpringLipidFluctuations::setup(int vflag)
{
...
void FixSpringLipidFluctuations::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixSpringLipidFluctuations::post_force(int vflag)
{
  spring_couple();
}
...
void FixSpringLipidFluctuations::spring_couple()
{
  double xcm[3],xcm2[3];
  ...
  double a0,a1,a2,a3,a4,a5,fprime;
  dx = xcm2[0] - xcm[0];
  dy = xcm2[1] - xcm[1];
  dz = xcm2[2] - xcm[2];
  if (!xflag) dx = 0.0;
...
void FixSpringLipidFluctuations::post_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixSpringLipidFluctuations::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixSpringLipidFluctuations::compute_scalar()
{
  return espring;
}
...
double FixSpringLipidFluctuations::compute_vector(int n)
{
  return ftotal[n];
}
