...
Fix(lmp, narg, arg)
{
    ...
    ssweight_flag = dssp_hdrgn_flag = p_ap_flag = water_flag = membrane_gamma_flag = membrane_helix_flag  = burial_flag = helix_flag = amh_go_flag = frag_mem_flag = vec_frag_mem_flag = 0;
    ssb_flag = frag_mem_tb_flag = vec_frag_mem_tb_flag = phosph_flag = memb_flag = 0;
    ...
    while (!in.eof()) {
        ...
        if (strcmp(varsection, "[ABC]")==0) {
        ...
        } else if (strcmp(varsection, "[Membrane_Gamma]")==0) {
            membrane_gamma_flag = 1;
            if (comm->me==0) print_log("Membrane_Gamma flag on\n");
            in >> membrane_k_water;
        ...
        } else if (strcmp(varsection, "[Membrane_Helix]")==0) {
            membrane_helix_flag = 1;
            if (comm->me==0) print_log("Membrane_Helix flag on\n");
            in >> membrane_k_helix;
            for (int j=0;j<20;++j)
            in >> membrane_h4prob[j];
            in >> membrane_pro_accepter_flag >> membrane_h4prob_pro_accepter;
            in >> membrane_helix_sigma_HO >> membrane_helix_sigma_NO;
            in >> membrane_helix_HO_zero >> membrane_helix_NO_zero;
            ...
            for (int j=0;j<4;++j)
            in >> g_memb[i][j];
        }else if (strcmp(varsection, "[partial]")==0) {
            partial_flag = 1;
            print_log("partial potential flag on\n");
            in >> startN;
            in >> endN;
    ...
    if (membrane_gamma_flag) {
        ifstream in_mg("membrane_gamma.dat");
        if (!in_mg) error->all(FLERR,"File membrane_gamma.dat doesn't exist");
        for (int i_well=0;i_well<n_wells;++i_well) {
            for (i=0;i<20;++i) {
                for (j=i;j<20;++j) {
                    in_mg >> membrane_gamma[i_well][i][j][0] >> membrane_gamma[i_well][i][j][1];
                    membrane_gamma[i_well][j][i][0] = membrane_gamma[i_well][i][j][0];
                    membrane_gamma[i_well][j][i][1] = membrane_gamma[i_well][i][j][1];
                }
            }
        }
        in_mg.close();
    ...
    if (amh_go_flag) {
        ...
        if (frustration_censoring_flag == 2) {
          ifstream in_rnativeCACA("go_rnativeCACA.dat");
          ifstream in_rnativeCBCB("go_rnativeCBCB.dat");
          ifstream in_rnativeCACB("go_rnativeCACB.dat");
...
inline double FixBackbone::get_water_gamma(int i_resno, int j_resno, int i_well, int ires_type, int jres_type, int water_prot_flag)
{
    ...
    if (!phosph_flag) {
        if(membrane_gamma_flag){
            //only consider no phosph case
            //I need membrane_gamma
            //determined by zim
            // 2 means transmembrane
            // 1 is outermembrane, 3 is hydrophilic
            if(z_res[i_resno] != 2 && z_res[j_resno] != 2){
                return water_gamma[i_well][ires_type][jres_type][water_prot_flag];
            }
            else{
                if(z_res[i_resno] == z_res[j_resno]){
                    return (membrane_k_water/k_water) * membrane_gamma[i_well][ires_type][jres_type][water_prot_flag];
                }
                else{
                    return ((membrane_k_water/k_water) * membrane_gamma[i_well][ires_type][jres_type][water_prot_flag] + water_gamma[i_well][ires_type][jres_type][water_prot_flag]) / 2.0;
                }
            }
        }
        else{
...
{
    if (R->rNO(i,j)>dssp_hdrgn_cut){
        // fprintf(dout, "---------LALALA-----------(%d %d %f) \n", i, j, R->rNO(i,j));
        return;
    }
...
void FixBackbone::compute_helix_potential(int i, int j)
{
    ...
    if (membrane_helix_flag) {
        ...
        if(z_res[i_resno] != 2 && z_res[j_resno] != 2){
            ...
            if(z_res[i_resno] == z_res[j_resno]){
                double h4probi = membrane_h4prob[ires_type];
                if (se[i_resno]=='P' && membrane_pro_accepter_flag) h4probi = membrane_h4prob_pro_accepter;

                prob_sum = h4probi + membrane_h4prob[jres_type]; // sequence-identity weight

                pair_theta = (membrane_k_helix/k_helix)*prob_sum*exp( - pow(R_NO - membrane_helix_NO_zero, 2)/(2.0*pow(membrane_helix_sigma_NO, 2)) - pow(R_HO - membrane_helix_HO_zero, 2)/(2.0*pow(membrane_helix_sigma_HO, 2)) );

                prd_pair_theta[0] = - (R_NO - membrane_helix_NO_zero)/(pow(membrane_helix_sigma_NO, 2)*R_NO);
                prd_pair_theta[1] = - (R_HO - membrane_helix_HO_zero)/(pow(membrane_helix_sigma_HO, 2)*R_HO);
            }
            else{
                double h4probi = membrane_h4prob[ires_type];
                if (se[i_resno]=='P' && membrane_pro_accepter_flag) h4probi = membrane_h4prob_pro_accepter;
...
void FixBackbone::compute_membrane_potential(int i)
{
    ...
    // V=(-g_memb[0][0]*s_per)+(-g_memb[0][1]*s_cyt);
    ...
    energy[ET_MEMB] += epsilon*k_overall_memb*V*z_res[i];
    ...
    // calculate forces
    memb_force_x = -epsilon*k_overall_memb*dV_dx*z_res[i];
    memb_force_y = -epsilon*k_overall_memb*dV_dy*z_res[i];
    memb_force_z = -epsilon*k_overall_memb*dV_dz*z_res[i];
