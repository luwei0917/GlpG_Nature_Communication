...
void FixSpringRGCylindricalZdependent::post_force(int vflag)
{
  ...
  double xcm[3];
  //
  group->xcm(igroup,masstotall,xcm);
  double rg;
  rg = group->z_based_cylindricalgyration(igroup,masstotall, xcm,memb_b, k_bin);
  rg_total = rg;
  //xxcm is defined as \sum m_i theta(z_i) x_i / M
  double xxcm[3];
  group->z_based_xcm(igroup,masstotall,xxcm, memb_b, k_bin);
  // group->xcm(igroup,masstotall,xxcm);
  double rg_old = group->cylindricalgyration(igroup,masstotall,xcm);
  ...
  double second_partX, second_partY;
  // z based mass is defined as \sum m_i theta(z_i)
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      dy = unwrap[1] - xcm[1];
      z = unwrap[2];
      rg += 1e-8;  //just to prevent rg considered to be zero
      term1 = 2.0 * k * (1.0 - rg0/rg);
      theta = 0.5*(tanh(k_bin*(z+memb_b))-tanh(k_bin*(z-memb_b)));
      if (rmass) massfrac = rmass[i]/masstotall;
      else  massfrac = mass[type[i]]/masstotall;
      theta_prime = 0.5*k_bin*(-pow(tanh(k_bin*(z+memb_b)),2) + pow(tanh(k_bin*(z-memb_b)),2));

      if (rmass) mass_theta_prime_frac = rmass[i]*theta_prime/masstotall;
      else  mass_theta_prime_frac = mass[type[i]]*theta_prime/masstotall;
      second_partX = xxcm[0] - masstotal * xcm[0]/masstotall;
      second_partY = xxcm[1] - masstotal * xcm[1]/masstotall;
      forceAddX = term1*(dx*theta - second_partX)*massfrac;
      forceAddY = term1*(dy*theta - second_partY)*massfrac;
      f[i][0] -= forceAddX;
      f[i][1] -= forceAddY;
      left = dx*dx+dy*dy;
      ...
      f[i][2] -= forceAddZ;
      // if(i == 10){
      //   fprintf(stderr,"The 10th forceZ is %f.\n", forceAddZ);
      // }
    }
